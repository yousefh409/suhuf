"""AI catalog enrichment for book and author metadata.

Both functions run through the shared OpenRouter client (:mod:`ingestion._client`),
which builds an Anthropic SDK client pointed at OpenRouter. Each asks Claude for a
JSON object and parses it tolerantly. When no client can be built offline (no
``OPENROUTER_API_KEY``) every call returns ``{}`` so the pipeline runs without an
API key.
"""
...
def enrich_book_metadata(result: ParseResult, client: "Anthropic | None" = None) -> dict:
    """Use Claude to enrich book metadata. Returns a dict of enriched fields, or {}."""
    if client is None:
        try:
            client = create_client()
        except Exception as e:
            logger.warning(f"Could not create enrichment client: {e}")
            return {}
...
def enrich_author_metadata(author_id: str, existing_data: dict,
                           client: "Anthropic | None" = None) -> dict:
    """Use Claude to enrich author metadata. Returns a dict of enriched fields, or {}."""
    if client is None:
        try:
            client = create_client()
        except Exception as e:
