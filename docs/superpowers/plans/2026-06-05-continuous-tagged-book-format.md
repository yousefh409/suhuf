...
- [ ] **Assembly.** Test first: assembling Nawawi pages yields continuous plain text and a correct page-offset table (page 49 and 50 offsets land mid-hadith-2). Implement assembly in the pipeline. Also derives unit boundaries (heading/number starts) and excludes page markers, feeding `chunk_text`.
